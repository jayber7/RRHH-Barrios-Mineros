...
const tipoColor = (tipo) =>
  tipo === 'RESIDENTE' ? 'bg-purple-100 text-purple-600'
  : tipo === 'CONTRATO' ? 'bg-blue-100 text-blue-600'
  : tipo === 'CONSULTORÍA' ? 'bg-amber-100 text-amber-600'
                ...
                {stats.topAtrasos?.map((emp, i) => (
                  <div key={emp.id} onClick={() => { setDetallePersonal(emp.id); setDetalleFecha(''); setDetalleData(null); setShowDetalle(true); }}
                    ...
                    <div className="text-right">
                      ...
                      <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${tipoColor(emp.tipo_planilla)}`}>{emp.tipo_planilla || 'SIN TIPO'}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            {stats.asistenciaPorPlanilla?.map((p, i) => (
              <div key={i} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className={`w-10 h-10 ${tipoColor(p.tipo_planilla)} rounded-xl flex items-center justify-center mb-3`}>
                  <Briefcase size={18} />
                </div>
                <p className="text-sm font-bold text-slate-700">{p.tipo_planilla || 'SIN TIPO'}</p>
